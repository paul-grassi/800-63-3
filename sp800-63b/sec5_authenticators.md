...
Look-up secret authenticators (the media and devices containing look-up secrets) SHALL be protected by the subscriber against theft or duplication. If the authenticator uses look-up secrets sequentially from a list, the subscriber MAY dispose of used secrets, but only after a successful authentication. The subscriber SHALL contact their CSP immediately if theft or duplication of the authenticator is suspected.
...
Out of band authenticators SHALL be protected by the subscriber against theft or loss. The subscriber SHALL contact their CSP immediately if loss or theft of the authenticator is suspected. ***This is getting repetitive -- perhaps goes in lifecycle section?***
