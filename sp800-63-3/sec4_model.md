...
Digital authentication begins with enrollment. The usual sequence for enrollment proceeds as follows. An Applicant applies to a CSP. If approved, the CSP binds an authenticator to an identifier, which can be pseudonymous (and possibly one or more attributes that the RA has verified), creates a credential, and issues the authenticator to the subscriber. The authenticator may be issued by the CSP, generated directly by the subscriber, or provided by a third party. The CSP registers the authenticator by creating a credential that binds the authenticator to an identifier and possibly other attributes that the CSP has verified. The authenticator and credential may be used in subsequent authentication events.
